...
describe("parseMarkdownNote", () => {
  it("preserves the note identity and separates author evidence from user observation", () => {
    const parsed = parseMarkdownNote("20_seeds/DINCO_LLM.md", dincoLikeMarkdown);
    expect(parsed.title).toContain("Calibrating Verbalized Confidence");
    expect(parsed.externalId).toBe("2509.25532v2");
    expect(parsed.sections.find(section => section.rawHeading === "저자 명시")?.sectionType).toBe("AUTHOR_LIMITATIONS");
    expect(parsed.sections.find(section => section.rawHeading === "내가 본 것")?.sectionType).toBe("USER_OBSERVATIONS");
    expect(parsed.sections.find(section => section.rawHeading === "내 맥락")?.sectionType).toBe("USER_CONTEXT");
    expect(parsed.warnings).toContain("section_missing:REVIEWER_CRITICISMS");
    expect(parsed.warnings.some(warning => warning.startsWith("unknown_heading"))).toBe(false);
  });

  it("parses DINCO colon subsections under a generic limitation heading", () => {
    const parsed = parseMarkdownNote("DINCO_LLM.md", "---\ntitle: DINCO\n---\n\n## 한계\n\n저자 명시:\n- 비용이 증가한다 (§4).\n\n내가 본 것:\n- 추가 검증이 필요하다. [외부지식]\n\n## 내 맥락\n(사용자 확인 필요) 요약됨.\n");
    expect(parsed.sections.find(section => section.rawHeading === "저자 명시")?.sectionType).toBe("AUTHOR_LIMITATIONS");
    expect(parsed.sections.find(section => section.rawHeading === "내가 본 것")?.sectionType).toBe("USER_OBSERVATIONS");
    expect(parsed.warnings).toContain("section_missing:REVIEWER_CRITICISMS");
  });
  ...
  it("indexes wikilinks, Markdown URLs, and paper identifiers with source locators", () => {
    const parsed = parseMarkdownNote("20_seeds/linked.md", "---\nid: 2509.25532v2\ntitle: Linked\n---\n\n## 주장\nSee [[DINCO_LLM|the note]] and [paper](https://arxiv.org/abs/2509.25532).\n");
    expect(parsed.links).toEqual(expect.arrayContaining([
      expect.objectContaining({ linkType: "IDENTIFIER", target: "2509.25532v2", sourceLocator: "frontmatter:id" }),
      expect.objectContaining({ linkType: "WIKILINK", target: "DINCO_LLM", label: "the note", sourceLocator: "line:7" }),
      expect.objectContaining({ linkType: "MARKDOWN_URL", target: "https://arxiv.org/abs/2509.25532", label: "paper", sourceLocator: "line:7" }),
    ]));
  });

  it("marks explicit empty sections differently from missing sections", () => {
    const parsed = parseMarkdownNote("empty.md", "---\ntitle: Empty\n---\n\n## 주장\n없음\n");
  ...
  it("rejects unsafe absolute and parent paths", () => {
    ...
    expect(normalizeRelativePath("20_seeds\\DINCO_LLM.md")).toBe("20_seeds/DINCO_LLM.md");
  });
});
