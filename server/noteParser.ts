...
const SECTION_ALIASES: Array<{ type: NoteSectionType; names: string[] }> = [
  ...
  {
    type: "USER_CONTEXT",
    names: ["내 주제와의 접점", "내 맥락", "user context", "research context"],
  },
  {
    type: "REPRODUCIBILITY",
    names: ["재현 정보", "재현성", "reproducibility", "reproduction"],
  },
];
...
const TRACKED_SECTION_TYPES: NoteSectionType[] = [
  ...CANONICAL_TYPES,
  "USER_OBSERVATIONS",
];
...
export function normalizeRelativePath(value: string): string {
  ...
  if (
    ...
    throw new Error("절대 경로는 업로드할 수 없습니다.");
  const segments = normalized.split("/").filter(Boolean);
  if (segments.some(segment => segment === ".."))
    throw new Error("상위 경로가 포함된 파일은 업로드할 수 없습니다.");
  const safeSegments = segments.filter(
    segment => segment !== "." && segment !== "__MACOSX"
  );
  if (!safeSegments.length) throw new Error("파일 경로를 확인할 수 없습니다.");
  const result = safeSegments.join("/");
  if (result.length > 512)
    throw new Error("파일 경로는 512자 이하만 허용됩니다.");
  return result;
}
...
function parseScalar(value: string): unknown {
  ...
  // YAML double-quoted scalars escape with a backslash, single-quoted ones by doubling the
  // quote. Returning the raw slice left `\"` visible in every title that contains a quote.
...
function parseFrontmatter(raw: string): {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    // A key with no inline value may open a YAML block sequence ("tags:\n  - a\n  - b"),
    // which Obsidian writes by default. Consume the following "- item" lines as an array.
    const items: unknown[] = [];
    ...
    index = lookahead - 1;
    metadata[key] = sawItem ? items : null;
  }
...
function resolveSectionType(rawHeading: string): NoteSectionType {
  ...
  return (
    SECTION_ALIASES.find(alias =>
      alias.names.some(name => normalizeHeading(name) === normalized)
    )?.type ?? "UNKNOWN"
  );
}
...
function parseInlineSubsections(
  body: string,
  startOrder: number
): ParsedNoteSection[] {
  const inlinePattern =
    /^(저자 명시|내가 본 것|사용자 관찰|reviewer criticisms|author limitations)\s*:\s*$/gm;
...
function extractLinks(
  ...
  for (const match of Array.from(raw.matchAll(wikilinkPattern))) {
    ...
    if (target)
      add({
        ...
        sourceLocator: sourceLocator(raw, match.index ?? 0),
      });
  }
  ...
  for (const key of ["doi", "arxiv", "id"]) {
    ...
    add({
      ...
      target: value.trim(),
      label: key,
      sourceLocator: `frontmatter:${key}`,
    });
  }
  return links;
}

export type SectionPresenceInput = Pick<
  ParsedNoteSection,
  "sectionType" | "body" | "explicitEmpty"
>;

/** Shared by the parser and by library reads so stored rows and fresh parses agree. */
export function buildSectionPresence(
  sections: readonly SectionPresenceInput[]
): Record<NoteSectionType, boolean> {
  ...
  for (const section of sections) {
    if (section.body.trim() || section.explicitEmpty)
      presence[section.sectionType] = true;
  }
  return presence;
}
...
export function parseMarkdownNote(sourcePath: string, raw: string): ParsedNote {
  ...
  const fallbackTitle =
    sourcePathOnly.split("/").at(-1)?.replace(/\.md$/i, "") || "Untitled note";
  const metadata = parsedFrontmatter.metadata;
  const title =
    typeof metadata.title === "string" && metadata.title.trim()
      ? metadata.title.trim()
      : fallbackTitle;
  const externalIdValue = [metadata.id, metadata.doi, metadata.arxiv].find(
    value =>
      typeof value === "string" ? value.trim() : typeof value === "number"
  );
  const externalId =
    externalIdValue === undefined ? null : String(externalIdValue).trim();
  const sections: ParsedNoteSection[] = [];
  if (parsedFrontmatter.frontmatter !== null) {
    sections.push({
      rawHeading: "frontmatter",
      sectionType: "FRONTMATTER",
      body: parsedFrontmatter.frontmatter,
      explicitEmpty: false,
      sectionOrder: 0,
    });
  }
  ...
  headings.forEach((heading, index) => {
    ...
    sections.push({
      ...
      sectionType,
      body,
      explicitEmpty: isExplicitEmpty(body),
      sectionOrder: sections.length,
    });
    if (normalizeHeading(rawHeading) === "한계")
      sections.push(...parseInlineSubsections(body, sections.length));
  });
  ...
  for (const section of sections) {
    if (
      ...
      section.rawHeading.toLocaleLowerCase("ko-KR") !== "한계"
    )
      warnings.push(`unknown_heading:${section.rawHeading}`);
  }
...
export function calculateSectionFillRates(
  notes: Array<Pick<ParsedNote, "sectionPresence" | "explicitEmptySections">>
) {
  ...
  return Object.fromEntries(
    types.map(type => [
      type,
      {
        ...
        evidence: notes.filter(
          note =>
            note.sectionPresence[type] &&
            !note.explicitEmptySections.includes(type)
        ).length,
        total: notes.length,
      },
    ])
