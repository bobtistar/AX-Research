...
async function getTopTierSourceIds(): Promise<string[]> {
  ...
  topTierSourceIdsPromise = Promise.all(
    TOP_TIER_VENUES.map(async venue => {
      ...
      return (
        (payload.results ?? []).find(source =>
          sourceMatchesVenue(source.display_name, venue)
        )?.id ?? null
      );
    })
  )
